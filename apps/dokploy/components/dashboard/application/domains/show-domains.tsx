		...
		try {
			...
			setValidationStates((prev) => ({
				...
				[host]: {
					isLoading: false,
					isValid: false,
					error: error.message || t("services.domains.toast.validateError"),
				},
			}));
		}
	};
			...
			<Card className="bg-background">
				<CardHeader className="flex flex-row items-center flex-wrap gap-4 justify-between">
					<div className="flex flex-col gap-1">
						<CardTitle className="text-xl">{t("form.domains")}</CardTitle>
						<CardDescription>
					...
					<div className="flex flex-row gap-4 flex-wrap">
						{data && data?.length > 0 && (
							<AddDomain id={id} type={type}>
								<Button>
									<GlobeIcon className="size-4" /> {t("domain.add")}
								</Button>
							</AddDomain>
						)}
					</div>
				...
				<CardContent className="flex w-full flex-row gap-4">
					{isLoadingDomains ? (
						...
						<div className="flex w-full flex-col items-center justify-center gap-3 min-h-[40vh]">
							...
							<span className="text-base text-muted-foreground">
								{t("services.domains.emptyDescription")}
							</span>
							<div className="flex flex-row gap-4 flex-wrap">
								<AddDomain id={id} type={type}>
									<Button>
											...
											<div className="flex flex-col gap-4">
												...
												<div className="flex items-center justify-between flex-wrap gap-y-2">
													...
													<div className="flex gap-2 flex-wrap">
														...
														<DialogAction
															title={t("services.domains.delete.title")}
															description={t(
																"services.domains.delete.description",
															)}
															type="destructive"
															onClick={async () => {
																await deleteDomain({
																	...
																	.then((_data) => {
																		refetch();
																		toast.success(
																			t("services.domains.toast.deleted"),
																		);
																	})
																	.catch(() => {
																		toast.error(
																			t("services.domains.toast.deleteError"),
																		);
																	});
															}}
														>
														...
														<Tooltip>
															<TooltipTrigger asChild>
																<Badge variant="secondary">
																	...
																	{t("services.domains.path")}:{" "}
																	{item.path || "/"}
																</Badge>
															</TooltipTrigger>
															<TooltipContent>
																<p>{t("services.domains.pathHelp")}</p>
															</TooltipContent>
														</Tooltip>
													</TooltipProvider>
													...
													<TooltipProvider>
														<Tooltip>
															<TooltipTrigger asChild>
																<Badge variant="secondary">
																	<InfoIcon className="size-3 mr-1" />
																	{t("form.port")}: {item.port}
																</Badge>
															</TooltipTrigger>
															<TooltipContent>
																<p>{t("services.domains.portHelp")}</p>
															</TooltipContent>
														</Tooltip>
													</TooltipProvider>
													...
													<TooltipProvider>
														<Tooltip>
															...
															<TooltipContent>
																<p>
																	{item.https
																		? t("services.domains.httpsSecure")
																		: t("services.domains.httpStandard")}
																</p>
															</TooltipContent>
														</Tooltip>
													</TooltipProvider>
															...
															<Tooltip>
																<TooltipTrigger asChild>
																	<Badge variant="outline">
																		{t("services.domains.certificate")}:{" "}
																		{item.certificateType}
																	</Badge>
																</TooltipTrigger>
																<TooltipContent>
																	<p>{t("services.domains.certificateHelp")}</p>
																</TooltipContent>
															</Tooltip>
														</TooltipProvider>
													)}
																...
																>
																	{validationState?.isLoading ? (
																		<>
																			...
																			{t("services.domains.checkingDns")}
																		</>
																	) : validationState?.isValid ? (
																		<>
																			...
																			validationState.cdnProvider
																				? t("services.domains.behindCdn", {
																						value: validationState.cdnProvider,
																					})
																				: t("services.domains.dnsValid")}
																		</>
																	) : validationState?.error ? (
																		<>
																		...
																		<>
																			...
																			{t("services.domains.validateDns")}
																		</>
																	)}
																</Badge>
															</TooltipTrigger>
																...
																{validationState?.error ? (
																	<div className="flex flex-col gap-1">
																		<p className="font-medium text-red-500">
																			{t("services.compose.logs.errorPrefix")}
																		</p>
																		<p>{validationState.error}</p>
																	</div>
																) : (
																	t("services.domains.validateDnsHelp")
																)}
															</TooltipContent>
														</Tooltip>
													</TooltipProvider>
