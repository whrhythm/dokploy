...
export const AddWorker = ({ serverId }: Props) => {
	...
	return (
		<CardContent className="sm:max-w-4xl   flex flex-col gap-4 px-0">
			<DialogHeader>
				<DialogTitle>{t("cluster.Modal.addWorker.title")}</DialogTitle>
				<DialogDescription>
					{t("cluster.Modal.addWorker.description")}
				</DialogDescription>
			</DialogHeader>
			...
			{isPending ? (
				...
				<>
					<div className="flex flex-col gap-2.5 text-sm">
						<span>{t("cluster.Modal.addWorker.step1")}</span>
					...
					<div className="flex flex-col gap-2.5 text-sm">
						<span>{t("cluster.Modal.addWorker.step2")}</span>
