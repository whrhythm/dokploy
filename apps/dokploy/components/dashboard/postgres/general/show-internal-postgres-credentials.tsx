				...
				<Card className="bg-background">
					<CardHeader>
						<CardTitle className="text-xl">
							{t("services.postgres.internalCredentials")}
						</CardTitle>
					</CardHeader>
					<CardContent className="flex w-full flex-row gap-4">
						<div className="grid w-full md:grid-cols-2 gap-4 md:gap-8">
							<div className="flex flex-col gap-2">
								<Label>{t("services.postgres.credentials.user")}</Label>
								<Input disabled value={data?.databaseUser} />
							</div>
							<div className="flex flex-col gap-2">
								<Label>{t("services.postgres.credentials.databaseName")}</Label>
								<Input disabled value={data?.databaseName} />
							</div>
							<div className="flex flex-col gap-2">
								<Label>{t("services.postgres.credentials.password")}</Label>
							...
							<div className="flex flex-col gap-2">
								<Label>{t("services.postgres.credentials.internalPort")}</Label>
								<Input disabled value="5432" />
							</div>

							<div className="flex flex-col gap-2">
								<Label>{t("services.postgres.credentials.internalHost")}</Label>
								<Input disabled value={data?.appName} />
							</div>

							<div className="flex flex-col gap-2">
								<Label>{t("services.postgres.credentials.internalUrl")}</Label>
								<ToggleVisibilityInput
									disabled
