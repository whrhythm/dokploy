...
export const UpdateRedis = ({ redisId }: Props) => {
	...
	const onSubmit = async (formData: UpdateRedis) => {
		await mutateAsync({
			...
			.then(() => {
				toast.success(t("services.redis.toast.updated"));
			...
			.catch(() => {
				toast.error(t("services.redis.toast.updateError"));
			})
			.finally(() => {});
	};
	...
	return (
		<Dialog open={isOpen} onOpenChange={setIsOpen}>
			...
			<DialogContent className="sm:max-w-lg">
				<DialogHeader>
					<DialogTitle>{t("pages.Modal.redisUpdate.title")}</DialogTitle>
					<DialogDescription>
						{t("pages.Modal.redisUpdate.description")}
					</DialogDescription>
				</DialogHeader>
						...
						<Form {...form}>
							...
							>
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>{t("services.redis.form.name")}</FormLabel>
											<FormControl>
												<Input
													placeholder={t("services.redis.form.namePlaceholder")}
													{...field}
												/>
											</FormControl>
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>
												{t("services.redis.form.description")}
											</FormLabel>
											<FormControl>
												<Textarea
													placeholder={t(
														"services.redis.form.descriptionPlaceholder",
													)}
