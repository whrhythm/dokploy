			...
			.on("error", (err) => {
				...
				if (err.level === "client-authentication") {
					...
					const friendlyMessage = [
						...
						"",
						"💡 Hints:",
						"  • Check that the SSH key you added in 小智Ops is the same one installed on the server (e.g. in ~/.ssh/authorized_keys).",
						"  • Try generating a new SSH key in 小智Ops and add only the public key to the server, then try again.",
					...
					const friendlyMessage = [
						...
						"  • If the server is in a private network, ensure 小智Ops can reach it (VPN, firewall rules, or correct security groups).",
...
if [ -x "$(command -v snap)" ]; then
    ...
    if [ "$SNAP_DOCKER_INSTALLED" = "true" ]; then
        echo " - Docker is installed via snap."
			echo "   Please note that 小智Ops does not support Docker installed via snap."
